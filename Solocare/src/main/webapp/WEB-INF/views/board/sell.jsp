...
<body>
	...
	<div style="padding-top: 20px;">
	   <c:if test="${currentPage > 1}">
	       <c:choose>
	           <c:when test="${not empty items and not empty text}">
	               <a href="search?category=sell&page=${currentPage - 1}&items=${items}&text=${text}">이전</a>
	           </c:when>
	           <c:otherwise>
	               <a href="board?category=sell&page=${currentPage - 1}">이전</a>
	           </c:otherwise>
	       </c:choose>
	   </c:if>
	   ...
	   <c:forEach begin="1" end="${totalPages}" var="i">
	       <c:choose>
	           ...
	           <c:otherwise>
	               <c:choose>
	                   <c:when test="${not empty items and not empty text}">
	                       <a href="search?category=sell&page=${i}&items=${items}&text=${text}">${i}</a>
	                   </c:when>
	                   <c:otherwise>
	                       <a href="board?category=sell&page=${i}">${i}</a>
	                   </c:otherwise>
	               </c:choose>
	           </c:otherwise>
	       </c:choose>
	   ...
	   <c:if test="${currentPage < totalPages}">
	       <c:choose>
	           <c:when test="${not empty items and not empty text}">
	               <a href="search?category=sell&page=${currentPage + 1}&items=${items}&text=${text}">다음</a>
	           </c:when>
	           <c:otherwise>
	               <a href="board?category=sell&page=${currentPage + 1}">다음</a>
	           </c:otherwise>
	       </c:choose>
	   </c:if>
	</div>
